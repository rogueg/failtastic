  ...
  def fetch_range(t0, t1, opts={})
    ...
    msgs.map do |fd|
      ...
      OpenStruct.new({
        ...
        :body => (fd.attr.find{|k,v| k =~ /BODY/} || []).last.encode('UTF-8', :invalid => :replace, :undef => :replace)
      })
    end
  end

  def fetch_body(msg_id)
    fetch([msg_id], "BODY.PEEK[1]")[0].attr["BODY[1]"].encode('UTF-8', :invalid => :replace, :undef => :replace)
  end
